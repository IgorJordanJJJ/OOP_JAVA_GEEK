...
public class Zoo {
    ...
    private List<Swimming> swims(){
        List<Swimming> res = new ArrayList<>();
        for(Animal animal: animals){
            if(animal instanceof Swimming){
                res.add((Swimming)animal);
            }
        }
        return res;
    }

    public Animal championSwim () {
        List<Swimming> start = swims();
        Swimming champ = start.get(0);
        for(Swimming swimmer: start){
            if(champ.speedSwim()<swimmer.speedSwim()){
                champ = swimmer;
            }
        }
        return (Animal)champ;
